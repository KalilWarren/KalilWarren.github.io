...
_wide, _tbl, _eta, _dec = generate_one_way_repeated_measures_anova(
...
_to_json({'type':'rma_anova','dataset':_wide.to_dict(orient='records'),'table':_tbl.to_dict(orient='records'),'decision':_dec,'eta_squared':float(_eta)})
`) as RmaAnovaResult;
}
